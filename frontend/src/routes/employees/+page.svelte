<script lang="ts">
  ...
  function toggleLicence(option: string) {
    if (form.licence.includes(option)) {
      form.licence = form.licence.filter(l => l !== option);
    } else {
      form.licence = [...form.licence, option];
    }
  }
  ...
  function handlePhotoUpload(e: Event) {
    ...
    if (file) {
      ...
      reader.onload = (ev) => {
        form.photo_url = ev.target?.result as string;
      };
  ...
  async function fetchData() {
    try {
      const [empRes, posRes, licRes] = await Promise.all([
        fetch('http://127.0.0.1:8081/api/employees'),
        fetch('http://127.0.0.1:8081/api/positions'),
        fetch('http://127.0.0.1:8081/api/licenses')
      ]);
  ...
  function openEditModal(emp: any) {
    ...
    form = {
      ...
      licence: Array.isArray(emp.licence) ? emp.licence : (emp.licence ? emp.licence.split(',') : []),
          ...
          <tr class="hover:bg-white/5 transition-colors group">
            <td class="px-8 py-4">
              <div class="w-10 h-10 rounded-xl bg-slate-800 border border-white/10 overflow-hidden flex items-center justify-center">
                {#if emp.photo_url}
                  <img src={emp.photo_url} alt={emp.name} class="w-full h-full object-cover" />
                {:else}
                  <span class="text-xs font-black text-slate-500">{emp.name[0].toUpperCase()}</span>
                {/if}
              </div>
            </td>
            ...
            <td class="px-8 py-5 text-right">
              <div class="flex justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onclick={() => openDetail(emp)} class="p-2 hover:bg-emerald-500/10 text-emerald-400 rounded-lg transition-colors" title="View Detail" aria-label="View Detail">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                </button>
                <button onclick={() => openEditModal(emp)} class="p-2 hover:bg-sky-500/10 text-sky-400 rounded-lg transition-colors" title="Edit" aria-label="Edit Karyawan">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </button>
                <button onclick={() => confirmDelete(emp)} class="p-2 hover:bg-rose-500/10 text-rose-400 rounded-lg transition-colors" title="Delete" aria-label="Hapus Karyawan">
...
{#if showModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
    <button class="absolute inset-0 bg-slate-950/80 backdrop-blur-sm cursor-default border-none" onclick={() => showModal = false} aria-label="Close modal"></button>
    <div class="glass-dark w-full max-w-4xl max-h-[90vh] rounded-[3rem] border border-white/10 shadow-2xl relative z-10 flex flex-col">
      <div class="p-10 pb-0">
        ...
        <div class="flex items-center space-x-6 mb-10 bg-white/5 p-6 rounded-3xl border border-white/5">
          <div class="relative group">
            <div class="w-24 h-24 rounded-[2rem] bg-slate-800 border-2 border-white/10 overflow-hidden flex items-center justify-center shadow-2xl">
              {#if form.photo_url}
                <img src={form.photo_url} alt="Preview" class="w-full h-full object-cover" />
              {:else}
            ...
            <label for="photo_upload" class="absolute -bottom-2 -right-2 w-10 h-10 bg-sky-500 hover:bg-sky-400 text-white rounded-2xl flex items-center justify-center cursor-pointer shadow-lg transition-all scale-90 group-hover:scale-100">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
              <input id="photo_upload" type="file" accept="image/*" class="hidden" onchange={handlePhotoUpload} />
            </label>
          </div>
          <div>
            <h4 class="text-sm font-black text-white uppercase tracking-widest mb-1">Pas Foto Karyawan</h4>
            <p class="text-[10px] text-slate-500 uppercase font-bold tracking-tighter">Format JPG/PNG, Max 2MB</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-10 pt-0 custom-scroll">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div>
            <label for="nik" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">NIK</label>
            <input id="nik" type="text" bind:value={form.nik} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500">
          </div>
          <div>
            <label for="name" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Nama Lengkap</label>
            <input id="name" type="text" bind:value={form.name} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500" required>
          </div>
          <div>
            <label for="ktp_number" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">KTP Number</label>
            <input id="ktp_number" type="text" bind:value={form.ktp_number} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500">
          </div>
          <div>
            <label for="gender" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Gender</label>
            <select id="gender" bind:value={form.gender} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500">
          ...
          <div>
            <label for="position" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Jabatan</label>
            <select id="position" bind:value={form.position} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500">
              <option value="" class="bg-[#1e293b]">-- Pilih Jabatan --</option>
              {#each positions as pos}
                <option value={pos.name} class="bg-[#1e293b]">{pos.name}</option>
              {/each}
            </select>
          </div>
          <div>
            <label for="department" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Departemen</label>
            <input id="department" type="text" bind:value={form.department} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500">
          </div>
          <div>
            <label for="company" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Perusahaan</label>
            <select id="company" bind:value={form.company} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white">
              <option value="SJS" class="bg-[#1e293b]">SJS (Satria Jaya Sultra)</option>
              <option value="PAMA" class="bg-[#1e293b]">PAMA (Pama Persada)</option>
            </select>
          </div>
          <div>
            <label for="contract_type" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Contract Type</label>
            <input id="contract_type" type="text" bind:value={form.contract_type} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white">
          </div>
          <div>
            <label for="batch_number" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Batch Number</label>
            <input id="batch_number" type="text" bind:value={form.batch_number} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500">
          </div>
          <div class="col-span-1 md:col-span-2">
            <div class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Licence (Multi-select)</div>
            <div class="flex flex-wrap gap-2 p-4 bg-white/5 border border-white/10 rounded-3xl">
              {#each licenceOptions as opt}
                <button 
                  type="button"
                  onclick={() => toggleLicence(opt)}
                  class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border {form.licence.includes(opt) ? 'bg-sky-500 border-sky-400 text-white shadow-lg shadow-sky-500/20' : 'bg-white/5 border-white/10 text-slate-400 hover:border-white/20'}"
                >
                  {opt}
                </button>
              {/each}
          ...
          <div>
            <label for="phone_number" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Phone Number</label>
            <input id="phone_number" type="text" bind:value={form.phone_number} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white">
          </div>
          <div>
            <label for="email" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Email</label>
            <input id="email" type="email" bind:value={form.email} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white">
          </div>
          <div class="col-span-1 md:col-span-2">
            <label for="full_address" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Full Address</label>
            <textarea id="full_address" bind:value={form.full_address} class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white resize-none h-24"></textarea>
          </div>
        </div>
      </div>
...
{#if showDetailModal && viewingEmployee}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
    <button class="absolute inset-0 bg-slate-950/90 backdrop-blur-md border-none" onclick={() => showDetailModal = false} aria-label="Close detail modal"></button>
    <div class="glass-dark w-full max-w-5xl max-h-[90vh] rounded-[3rem] border border-white/10 shadow-2xl relative z-10 flex flex-col overflow-hidden">
      ...
      <div class="p-10 bg-gradient-to-br from-sky-500/10 to-transparent flex justify-between items-start">
        <div class="flex items-center space-x-8">
          <div class="w-24 h-24 rounded-3xl bg-slate-800 border-2 border-sky-500/30 overflow-hidden flex items-center justify-center shadow-xl shadow-sky-500/10">
            {#if viewingEmployee?.photo_url}
              <img src={viewingEmployee.photo_url} alt={viewingEmployee.name} class="w-full h-full object-cover" />
            {:else}
              <span class="text-4xl font-black text-sky-400">{viewingEmployee?.name?.[0]?.toUpperCase() || '?'}</span>
            {/if}
          </div>
          <div class="space-y-2">
        ...
        <button onclick={() => showDetailModal = false} class="p-4 bg-white/5 hover:bg-white/10 text-white rounded-2xl transition-all" aria-label="Close detail modal">
          ...
          <div class="space-y-8">
            <div>
              ...
              <div class="space-y-4">
                ...
                <!-- New Batch & Licence Fields -->
                <div class="p-6 bg-sky-500/5 border border-sky-500/10 rounded-2xl space-y-4">
                  <div>
                    <span class="text-[9px] font-bold text-sky-500/60 uppercase block mb-1">Batch Number</span>
                    <span class="text-sm font-black text-white">{viewingEmployee?.batch_number || '-'}</span>
                  </div>
                  <div>
                    <span class="text-[9px] font-bold text-sky-500/60 uppercase block mb-2">Licence</span>
                    <div class="flex flex-wrap gap-2">
                      {#if viewingEmployee?.licence && viewingEmployee.licence.length > 0}
                        {#each viewingEmployee.licence as lic}
                          <span class="px-2 py-1 bg-sky-500/20 text-sky-400 text-[10px] font-black rounded-md border border-sky-500/30">{lic}</span>
                        {/each}
                      {:else}
...
{#if showDeleteModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in zoom-in duration-200">
    <button class="absolute inset-0 bg-slate-950/80 backdrop-blur-sm border-none" onclick={() => showDeleteModal = false} aria-label="Close delete modal"></button>
